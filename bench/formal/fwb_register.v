...
module fwb_register #(
  ...
  always @(posedge i_clk or posedge i_reset)
    if (i_reset) freg <= i_register;
    else if (i_wb_stb && i_wb_we && i_wb_addr == ADDR) begin
      if (i_wb_sel[0]) freg[7:0] <= i_wb_data[7:0];
      if (i_wb_sel[1]) freg[15:8] <= i_wb_data[15:8];
      if (i_wb_sel[2]) freg[23:16] <= i_wb_data[23:16];
      if (i_wb_sel[3]) freg[31:24] <= i_wb_data[31:24];
    end
  // }}}

  // Comparing freg against i_register
  // {{{
  always @(posedge i_clk) if (!i_reset) assert (((freg ^ i_register) & MASK) == 0);
  // }}}
  ...
  always @(posedge i_clk)
    if (!i_reset && $past(!i_reset && i_wb_stb))
      assert (i_wb_ack);
      else if (!i_reset) assert (!i_wb_ack);
  // }}}

  // Verifying i_wb_return
  // {{{
  always @(posedge i_clk)
    if (!i_reset && $past(!i_reset && i_wb_stb && !i_wb_we && i_wb_addr == ADDR))
