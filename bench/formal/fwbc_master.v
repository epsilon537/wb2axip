...
module fwbc_master #(
  ...
  assign f_request = {i_wb_stb, i_wb_we, i_wb_addr, i_wb_data, i_wb_sel, i_wb_cti, i_wb_bte};
  ...
  always @(posedge i_clk)
    if ((f_past_valid) && (!$past(
      ...
      if (i_wb_we) `SLAVE_ASSUME($stable(i_wb_data));
    end
  // }}}
  // }}}
  ////////////////////////////////////////////////////////////////////////
  ...
  // {{{
  always @(posedge i_clk)
    if ((f_past_valid) && (!$past(i_wb_stb)) && (!i_wb_stb)) begin
      `SLAVE_ASSERT(!i_wb_ack);
      `SLAVE_ASSERT(!i_wb_err);
      `SLAVE_ASSERT(!i_wb_rty);
    end else if (f_past_valid && $past(i_wb_ack | i_wb_err | i_wb_rty) && !i_wb_stb) begin
      `SLAVE_ASSERT(!i_wb_ack);
      `SLAVE_ASSERT(!i_wb_err);
      `SLAVE_ASSERT(!i_wb_rty);
    end
  ...
  always @(posedge i_clk)
    if (f_past_valid && !$past(
            ...
            i_wb_stb && !(i_wb_ack | i_wb_err | i_wb_rty)
        ))
      `SLAVE_ASSUME(i_wb_stb);

  always @(posedge i_clk)
    if (!OPT_BUS_ABORT && f_past_valid && !$past(
            i_reset
        ) && $past(
            i_wb_stb
        ) && !$past(
  ...
  // {{{
  always @(*) begin
    if (i_wb_ack) `SLAVE_ASSERT(!i_wb_err && !i_wb_rty);
    else if (i_wb_err) `SLAVE_ASSERT(!i_wb_rty);
  end
  // }}}
  // }}}
  ////////////////////////////////////////////////////////////////////////
  ...
  always @(posedge i_clk) next_addr <= i_wb_addr + 1;
  ...
  always @(posedge i_clk)
    if (f_past_valid && !$past(
      ...
      `SLAVE_ASSUME(i_wb_stb);
      if ($past(i_wb_cti) == C_FIXED) begin
        `SLAVE_ASSUME($stable(i_wb_addr));
        `SLAVE_ASSUME((i_wb_cti == C_FIXED || i_wb_cti == C_EOB));
      end else if ($past(i_wb_cti == C_INCR)) begin
        `SLAVE_ASSUME($stable(i_wb_bte));
        case (i_wb_bte)
  ...
  generate
    if (F_MAX_DELAY > 0) begin : DELAYCOUNT
      //
      ...
      always @(*) if (i_wb_cyc) `SLAVE_ASSERT(f_stall_count < F_MAX_DELAY);
    end
  endgenerate
  // }}}
  ////////////////////////////////////////////////////////////////////////
  ...
  initial ack_count = 0;
  always @(posedge i_clk)
    if (i_reset || !i_wb_cyc) ack_count <= 0;
    else if (f_past_valid && i_wb_ack) ack_count <= ack_count + 1'b1;

  always @(*) cover (!i_wb_cyc && ack_count > 4);
  always @(*) cover (!i_wb_cyc && ack_count > 3);
  // }}}
endmodule
//
