...
module fwb_master #(
    ...
    localparam DLYBITS = (MAX_DELAY < 4) ? 2 : (MAX_DELAY >= 65536) ? 32 : $clog2(MAX_DELAY + 1),
    //
    ...
    output reg  [(F_LGDEPTH-1):0] f_nreqs,
    f_nacks,
  ...
  always @(*) if (!f_past_valid) `SLAVE_ASSUME(i_reset);
  // }}}
  ////////////////////////////////////////////////////////////////////////
  //
...
`endif
  ...
  always @(*) if (!f_past_valid) `SLAVE_ASSUME(!i_wb_cyc);
  // }}}
  ////////////////////////////////////////////////////////////////////////
  //
  // Bus requests
  ...
  always @(posedge i_clk)
    if (f_past_valid && !OPT_BUS_ABORT && $past(!i_reset && i_wb_stb && i_wb_stall))
      `SLAVE_ASSUME(i_wb_cyc);

  // STB can only be true if CYC is also true
  always @(*) if (i_wb_stb) `SLAVE_ASSUME(i_wb_cyc);
  ...
  always @(posedge i_clk)
    if ((f_past_valid) && (!$past(
      ...
      if (i_wb_we) `SLAVE_ASSUME(i_wb_data == $past(i_wb_data));
    end
  ...
  // low on this clock.
  always @(posedge i_clk)
    if ((f_past_valid) && (!$past(i_wb_cyc)) && (!i_wb_cyc)) begin
      `SLAVE_ASSERT(!i_wb_ack);
      `SLAVE_ASSERT(!i_wb_err);
  ...
  always @(posedge i_clk)
    if ((f_past_valid) && (!$past(i_reset)) && ($past(i_wb_cyc)) && (!i_wb_cyc)) begin
  ...
  always @(*) `SLAVE_ASSERT((!i_wb_ack) || (!i_wb_err));
  // }}}
  ////////////////////////////////////////////////////////////////////////
  //
  // Stall checking
  ...
  //
  generate
    if (F_MAX_STALL > 0) begin : MXSTALL
      //
      ...
      always @(posedge i_clk)
        if ((!i_reset) && (i_wb_stb) && (i_wb_stall)) f_stall_count <= f_stall_count + 1'b1;
        else f_stall_count <= 0;

      always @(*) if (i_wb_cyc) `SLAVE_ASSERT(f_stall_count < F_MAX_STALL);
    end
  endgenerate
  // }}}
  ////////////////////////////////////////////////////////////////////////
  ...
  generate
    if (F_MAX_ACK_DELAY > 0) begin : MXWAIT
      //
  ...
  initial f_nreqs = 0;
  always @(posedge i_clk)
    if ((i_reset) || (!i_wb_cyc)) f_nreqs <= 0;
    else if ((i_wb_stb) && (!i_wb_stall)) f_nreqs <= f_nreqs + 1'b1;
  ...
  always @(posedge i_clk)
    ...
    else if (!i_wb_cyc) f_nacks <= 0;
    else if ((i_wb_ack) || (i_wb_err)) f_nacks <= f_nacks + 1'b1;
  ...
  always @(*)
    if ((i_wb_cyc) && (F_MAX_REQUESTS > 0)) begin
      if (i_wb_stb) begin
        `SLAVE_ASSUME(f_nreqs < F_MAX_REQUESTS);
      end else `SLAVE_ASSUME(f_nreqs <= F_MAX_REQUESTS);
      `SLAVE_ASSERT(f_nacks <= f_nreqs);
      assert (f_outstanding < (1 << F_LGDEPTH) - 1);
    end else assume (f_outstanding < (1 << F_LGDEPTH) - 1);

  always @(*)
    if ((i_wb_cyc) && (f_outstanding == 0)) begin
    ...
    end else if (!i_wb_cyc && f_nacks == f_nreqs) begin
      `SLAVE_ASSERT(!i_wb_ack);
      `SLAVE_ASSERT(!i_wb_err);
    end
  // }}}
  ...
  generate
    if (!F_OPT_RMW_BUS_OPTION) begin
      ...
      always @(*) if (f_outstanding == 0) `SLAVE_ASSUME((i_wb_stb) || (!i_wb_cyc));
  ...
  generate
    if ((!F_OPT_DISCONTINUOUS) && (!F_OPT_RMW_BUS_OPTION)) begin : INSIST_ON_NO_DISCONTINUOUS_STBS
  ...
  generate
    if (F_OPT_SHORT_CIRCUIT_PROOF) begin
