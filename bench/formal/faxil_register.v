...
module faxil_register #(
  ...
  always @(*) assert ((MASK & FIXED_BIT_MASK) == 0);
  // }}}
  ...
  always @(*) begin
    ...
    if (S_AXIL_WSTRB[ik]) non_ro_write[ik*8+:8] = S_AXIL_WDATA[ik*8+:8];

    non_ro_write = (non_ro_write & ~FIXED_BIT_MASK) | (f_reg & FIXED_BIT_MASK);
  end

  generate
    if (OPT_ASYNC_RESET) begin : OPT_ASYNC
      always @(posedge S_AXI_ACLK or negedge S_AXI_ARESETN)
        if (!S_AXI_ARESETN) f_reg <= i_register;
        else if (S_AXIL_AWW && S_AXIL_AWADDR[AW-1:AXILLSB] == ADDR[AW-1:AXILLSB]) begin
          f_reg <= non_ro_write;
        end
      ...
      always @(posedge S_AXI_ACLK) last_reset <= !S_AXI_ARESETN;

      always @(posedge S_AXI_ACLK)
        if (last_reset) f_reg <= i_register;
        else if (S_AXIL_AWW && S_AXIL_AWADDR[AW-1:AXILLSB] == ADDR[AW-1:AXILLSB]) begin
          f_reg <= non_ro_write;
        end
  ...
  always @(posedge S_AXI_ACLK) if (S_AXI_ARESETN && $past(S_AXI_ARESETN)) assert (error_mask == 0);
  // }}}
  ...
  // properly.
  always @(posedge S_AXI_ACLK)
    if (!f_past_valid || !$past(S_AXI_ARESETN) || (OPT_ASYNC_RESET && !S_AXI_ARESETN))
      assert (!S_AXIL_BVALID || (!f_past_valid && !OPT_INITIAL));
      else if ($past(S_AXIL_AWW && S_AXIL_AWADDR[AW-1:AXILLSB] == ADDR[AW-1:AXILLSB]))
        assert (S_AXIL_BVALID);

  always @(posedge S_AXI_ACLK)
    if (!f_past_valid || !$past(S_AXI_ARESETN) || (OPT_ASYNC_RESET && !S_AXI_ARESETN))
      assert (!S_AXIL_RVALID || (!f_past_valid && !OPT_INITIAL));
      else if ($past(S_AXIL_AR && S_AXIL_ARADDR[AW-1:AXILLSB] == ADDR[AW-1:AXILLSB]))
  ...
  // {{{
  always @(posedge S_AXI_ACLK)
    if (S_AXI_ARESETN && $past(
            S_AXI_ARESETN && S_AXIL_AR && S_AXIL_ARADDR[AW-1:AXILLSB] == ADDR[AW-1:AXILLSB]
        ))
