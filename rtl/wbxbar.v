...
module wbxbar #(
  ...
  generate
    genvar N, M;
    for (N = 0; N < NM; N = N + 1) begin : DECODE_REQUEST
      // {{{
      // Register declarations
      // {{{
...
`endif
    ...
    for (N = NM; N < NMFULL; N = N + 1) begin : UNUSED_MASTER_SIGNALS
      // {{{
  ...
  generate
    for (M = 0; M < NS; M = M + 1) begin : SLAVE_GRANT
      // {{{
...
`ifdef REGISTERED_SGRANT
      ...
      always @(*) begin
        ...
        if (!request[sindex[M]][M] && m_stb[sindex[M]] && mempty[sindex[M]]) drop_sgrant = 1;
        if (!sgrant[M]) drop_sgrant = 0;
        if (i_reset) drop_sgrant = 1;
      end
      // }}}
      ...
      always @(posedge i_clk) begin
        ...
        if (request[iN][M] && (!mgrant[iN] || mempty[iN])) sgrant[M] <= 1;
        if (drop_sgrant) sgrant[M] <= 0;
      end
      // }}}
      // }}}
`else
      ...
      always @(*) begin
        sgrant[M] = 0;
        for (iN = 0; iN < NM; iN = iN + 1) if (grant[iN][M]) sgrant[M] = 1;
      end
      // }}}
      // }}}
`endif
    ...
    for (M = NS; M < NSFULL; M = M + 1) begin : UNUSED_SLAVE_SIGNALS
      // {{{
      assign s_data[M]  = 0;
      assign s_stall[M] = 1;
      assign s_ack[M]   = 0;
  ...
  // channel
  generate
    for (N = 0; N < NM; N = N + 1) begin : ARBITRATE_REQUESTS
      // {{{
      ...
      always @(*) begin
        ...
        if (mgrant[N] && !mempty[N]) stay_on_channel = 1;
      end
      // }}}
      ...
      always @(*) begin
        ...
        if (request[N][NS]) requested_channel_is_available = 1;

        if (NM < 2) requested_channel_is_available = m_stb[N];
      end
      // }}}
...
`ifdef NEW_MINDEX_CODE
        ...
        always @(*) begin
          ...
          for (iM = 0; iM < NS; iM = iM + 1) begin
            if (grant[N][iM])
              ...
              r_regrant[iM] = 1'b1;
            else if (!sgrant[iM] && !requested[N][iM]) r_regrant[iM] = 1'b1;

            if (!request[N][iM]) r_regrant[iM] = 1'b0;
          end

          if (grant[N][NS]) r_regrant[NS] = 1;
          if (!request[N][NS]) r_regrant[NS] = 0;

          if (mgrant[N] && !mempty[N]) r_regrant = 0;
        end
        // }}}
        ...
        always @(*) begin
          ...
          if (r_regrant[iM]) r_reindex = r_reindex | iM[LGNS-1:0];
          if (regrant == 0) r_reindex = r_mindex;
        end
        // }}}
...
`else
        // {{{
        always @(posedge i_clk)
          if (!mgrant[N] || mempty[N]) begin

            for (iM = 0; iM < NS; iM = iM + 1) begin
              if (request[N][iM] && grant[N][iM]) begin
                // Maintain any open channels
                r_mindex <= iM;
              end else if (request[N][iM] && !sgrant[iM] && !requested[N][iM]) begin
                // Open a new channel
                // if necessary
                r_mindex <= iM;
              end
...
`endif  // NEW_MINDEX_CODE
    ...
    for (N = NM; N < NMFULL; N = N + 1) begin : UNUSED_MINDEXES
      // {{{
      assign mindex[N] = 0;
      // }}}
    end
  ...
  generate
    for (M = 0; M < NS; M = M + 1) begin : GEN_SINDEX
      // {{{
...
`ifdef NEW_SINDEX_CODE
        ...
        always @(*) begin
          ...
          for (iN = 0; iN < NM; iN = iN + 1) begin
            ...
            if (grant[iN][M]) regrant[iN] = 1;
            else if (!sgrant[M] && !requested[iN][M]) regrant[iN] = 1;

            if (!request[iN][M]) regrant[iN] = 0;
            if (mgrant[iN] && !mempty[iN]) regrant[iN] = 0;
          end
        end
        ...
        always @(*) begin
          ...
          // per bit though
          if (regrant == 0) reindex = sindex[M];
          else for (iN = 0; iN < NM; iN = iN + 1) if (regrant[iN]) reindex = reindex | iN[LGNM-1:0];
        end
...
`else
        ...
        always @(posedge i_clk)
          for (iN = 0; iN < NM; iN = iN + 1) begin
            if (!mgrant[iN] || mempty[iN]) begin
              if (request[iN][M] && grant[iN][M]) r_sindex <= iN;
              else if (request[iN][M] && !sgrant[M] && !requested[iN][M]) r_sindex <= iN;
            end
          end
...
`endif
    ...
    for (M = NS; M < NSFULL; M = M + 1) begin : UNUSED_SINDEXES
      // {{{
  ...
  generate
    for (M = 0; M < NS; M = M + 1) begin : GEN_CYC_STB
      // {{{
      ...
      always @(posedge i_clk) begin
        if (sgrant[M]) begin
          ...
          if (!i_mcyc[sindex[M]]) begin
            ...
            if (!o_sstb[M] || !s_stall[M]) o_sstb[M] <= request[sindex[M]][M] && !mfull[sindex[M]];
          end
        end else begin
          o_scyc[M] <= 1'b0;
          o_sstb[M] <= 1'b0;
  ...
  generate
    if ((NM == 1) && (!OPT_LOWPOWER)) begin : ONE_MASTER
      ...
      // here.
      for (M = 0; M < NS; M = M + 1) begin : FOREACH_SLAVE_PORT
        always @(*) begin
          o_swe[M]             = r_swe;
      ...
      for (M = 0; M < NS; M = M + 1) begin : GEN_DOWNSTREAM
        // {{{
        always @(posedge i_clk) begin
          if (OPT_LOWPOWER && !sgrant[M]) begin
            ...
            if (OPT_LOWPOWER && !m_we[sindex[M]]) o_sdata[M*DW+:DW] <= 0;
            else o_sdata[M*DW+:DW] <= m_data[sindex[M]];
  ...
  generate
    if (OPT_DBLBUFFER) begin : DOUBLE_BUFFERRED_STALL
      ...
      for (N = 0; N < NM; N = N + 1) begin : FOREACH_MASTER_PORT
        // m_stall isn't buffered, since it depends upon
        // the already existing buffer within the address
        // decoder
        always @(*) begin
          if (grant[N][NS]) m_stall[N] = 1;
          else if (mgrant[N] && request[N][mindex[N]]) m_stall[N] = mfull[N] || s_stall[mindex[N]];
          else m_stall[N] = m_stb[N];

          if (o_merr[N]) m_stall[N] = 0;
        end
  ...
  generate
    for (N = 0; N < NM; N = N + 1) begin : COUNT_PENDING_TRANSACTIONS
      // {{{
...
`ifdef FORMAL
  ...
  generate
    for (N = 0; N < NM; N = N + 1) begin : GRANT_CHECKING
      // {{{
      reg checkgrant;
      ...
      for (M = 1; M <= NS; M = M + 1) begin
        // Can't grant two channels to the same master
        always @(*) if (f_past_valid && grant[N][M]) assert (grant[N][M-1:0] == 0);
      end
      ...
      always @(*)
        if (f_past_valid) begin
          checkgrant = 0;
          for (iM = 0; iM < NS; iM = iM + 1) if (grant[N][iM]) checkgrant = 1;
          if (grant[N][NS]) checkgrant = 1;
  ...
  generate
    for (N = 0; N < NM; N = N + 1) begin : CHECK_GRANTS
      // {{{
      for (M = 0; M < NS; M = M + 1) begin
        always @(*)
          if ((f_past_valid) && grant[N][M]) begin
            assert (mgrant[N]);
            assert (mindex[N] == M);
            assert (sgrant[M]);
            assert (sindex[M] == N);
  ...
  generate
    for (M = 0; M < NS; M = M + 1) begin : CHECK_SGRANT
      // {{{
      reg f_sgrant;
      ...
      always @(*) begin
        f_sgrant = 0;
        for (iN = 0; iN < NM; iN = iN + 1) if (grant[iN][M]) f_sgrant = 1;
      end

      always @(*) assert (sgrant[M] == f_sgrant);
      // }}}
    end
  endgenerate
  ...
  generate
    for (N = 0; N < NM; N = N + 1) begin : F_CHECK_TIMEOUT
      // {{{
      always @(*)
        if (f_past_valid) begin
...
`ifdef VERIFIC
  ...
  //
  always @(*) if (!f_past_valid) assume (sgrant == 0);
...
`endif
  ...
  //
  generate
    for (N = 0; N < NM; N = N + 1) begin : WB_SLAVE_CHECK
      // {{{
      fwb_slave #(
          .AW(AW),
          .DW(DW),
  ...
  generate
    for (M = 0; M < NS; M = M + 1) begin : WB_MASTER_CHECK
      // {{{
      fwb_master #(
          .AW(AW),
          .DW(DW),
  ...
  //
  generate
    for (N = 0; N < NM; N = N + 1) begin : CHECK_OUTSTANDING
      // {{{
      always @(*) if (mfull[N]) assert (m_stall[N]);

      always @(posedge i_clk)
        if (i_mcyc[N])
          assert (f_moutstanding[N] == w_mpending[N] + ((OPT_BUFFER_DECODER & dcd_stb[N]) ? 1 : 0));
      ...
      always @(posedge i_clk)
        if (i_mcyc[N] && !mgrant[N] && !o_merr[N])
          assert (f_moutstanding[N] == ((OPT_BUFFER_DECODER & dcd_stb[N]) ? 1 : 0));

          else if (i_mcyc[N] && mgrant[N] && !i_reset)
            for (iM = 0; iM < NS; iM = iM + 1)
              if (grant[N][iM] && o_scyc[iM] && !i_serr[iM] && !o_merr[N])
                assert (n_outstanding == {1'b0, f_soutstanding[iM]} + (o_sstb[iM] ? 1 : 0));

      always @(*)
        if (!i_reset) begin
          for (iM = 0; iM < NS; iM = iM + 1)
          if (grant[N][iM] && i_mcyc[N]) begin
            if (f_soutstanding[iM] > 0) assert (i_mwe[N] == o_swe[iM]);
            if (o_sstb[iM]) assert (i_mwe[N] == o_swe[iM]);
            if (o_mack[N]) assert (i_mwe[N] == o_swe[iM]);
            if (o_scyc[iM] && i_sack[iM]) assert (i_mwe[N] == o_swe[iM]);
            if (o_merr[N] && !timed_out[N]) assert (i_mwe[N] == o_swe[iM]);
            if (o_scyc[iM] && i_serr[iM]) assert (i_mwe[N] == o_swe[iM]);
          end
        end

      always @(*)
        if (!i_reset && OPT_BUFFER_DECODER && i_mcyc[N]) begin
          if (dcd_stb[N]) assert (i_mwe[N] == m_we[N]);
        end
      // }}}
    end
  endgenerate
  ...
  generate
    for (M = 0; M < NS; M = M + 1) begin : ASSERT_NOT_CYC_WO_GRANT
      // {{{
      always @(posedge i_clk) if (!$past(sgrant[M])) assert (!o_scyc[M]);
      // }}}
    end
  endgenerate
  // }}}
  ...
  always @(*) if (NS <= 1) assume (special_slave == 0);
  ...
  always @(*) begin
    special_slave = NS;
    for (iM = 0; iM < NS; iM = iM + 1) begin
      if (((special_address ^ SLAVE_ADDR[iM*AW+:AW]) & SLAVE_MASK[iM*AW+:AW]) == 0)
        special_slave = iM;
    end
  end

  generate
    if (NS > 1) begin : DOUBLE_ADDRESS_CHECK
      // {{{
      //
      ...
      always @(*)
        if (!f_past_valid) begin
          address_found = 0;
          for (iM = 0; iM < NS; iM = iM + 1) begin
            if (((special_address ^ SLAVE_ADDR[iM*AW+:AW]) & SLAVE_MASK[iM*AW+:AW]) == 0) begin
...
`ifdef VERIFIC
  always @(*) if (!f_past_valid) assume (special_value == 0);
`else
  initial assume (special_value == 0);
`endif
  always @(posedge i_clk)
    if (special_slave < NS) begin
      // {{{
      if ($past(o_sstb[special_slave] && !i_sstall[special_slave])) begin
        assume (i_sack[special_slave]);

        if ($past(
                !o_swe[special_slave]
            ) && ($past(
                o_saddr[special_slave*AW+:AW]
            ) == special_address))
          assume (i_sdata[special_slave*DW+:DW] == special_value);
      end else assume (!i_sack[special_slave]);
      assume (!i_serr[special_slave]);

      if (o_scyc[special_slave]) assert (f_soutstanding[special_slave] == i_sack[special_slave]);

      if (o_sstb[special_slave] && !i_sstall[special_slave] && o_swe[special_slave]) begin
  ...
  begin
    ...
    begin
      ...
      if (f_read_seq[0] && !$past(s_stall[special_slave])) begin
      ...
      end else if (f_read_seq[0] && $past(s_stall[special_slave])) begin
      ...
      if (f_read_seq[1] && $past(s_stall[special_slave])) begin
        assert (o_scyc[special_slave]);
        assert (o_sstb[special_slave]);
        assert (!o_swe[special_slave]);
        assert (o_saddr[special_slave*AW+:AW] == special_address);
        f_read_seq[1] <= 1;
      end else if (f_read_seq[1] && !$past(s_stall[special_slave])) begin
        assert (i_sack[special_slave]);
        assert (i_sdata[special_slave*DW+:DW] == $past(special_value));
        if (OPT_DBLBUFFER) f_read_seq[2] <= 1;
      end

      if (f_read_ack) begin
        assert (o_mack[special_master]);
        assert (o_mdata[special_master*DW+:DW] == $past(special_value, 2));
      end
    end else f_read_seq <= 0;

  always @(*) cover (i_mcyc[special_master] && f_read_ack);
  ...
  always @(*)
    f_write_ack = (f_write_seq[2] || ((!OPT_DBLBUFFER) && f_write_seq[1] && !f_write_sstall));
  ...
  begin
    ...
    begin
      ...
      if (f_write_seq[0] && !$past(s_stall[special_slave])) begin
        assert (o_scyc[special_slave]);
        assert (o_sstb[special_slave]);
        assert (o_swe[special_slave]);
        assert (o_saddr[special_slave*AW+:AW] == special_address);
        assert (o_sdata[special_slave*DW+:DW] == $past(m_data[special_master]));
        assert (o_ssel[special_slave*DW/8+:DW/8] == $past(m_sel[special_master]));

        f_write_seq[1] <= 1;

      end else if (f_write_seq[0] && $past(s_stall[special_slave])) begin
        assert ($stable(m_stb[special_master]));
        assert (m_we[special_master]);
        assert (m_addr[special_master] == special_address);
      ...
      if (f_write_seq[1] && $past(s_stall[special_slave])) begin
        assert (o_scyc[special_slave]);
        assert (o_sstb[special_slave]);
        assert (o_swe[special_slave]);
        assert (o_saddr[special_slave*AW+:AW] == special_address);
        ...
        f_write_seq[1] <= 1;
      end else if (f_write_seq[1] && !$past(s_stall[special_slave])) begin
        for (iM = 0; iM < DW / 8; iM = iM + 1) begin
          if ($past(o_ssel[special_slave*DW/8+iM]))
            assert (special_value[iM*8+:8] == $past(o_sdata[special_slave*DW+iM*8+:8]));
        end

        assert (i_sack[special_slave]);
        if (OPT_DBLBUFFER) f_write_seq[2] <= 1;
      end

      if (f_write_seq[2] || ((!OPT_DBLBUFFER) && f_write_seq[1] && !$past(s_stall[special_slave])))
        assert (o_mack[special_master]);
    end else f_write_seq <= 0;

  always @(*) cover (i_mcyc[special_master] && f_write_ack);
  // }}}
  ////////////////////////////////////////////////////////////////////////
  //
  ...
  always @(posedge i_clk)
    if (!f_past_valid || i_reset) f_cvr_aborted <= 0;
    else
      for (iN = 0; iN < NM; iN = iN + 1) begin
        if (request[iN][NS]) f_cvr_aborted = 1;
        if ($fell(i_mcyc[iN])) begin
          if (f_macks[iN] != f_mreqs[iN]) f_cvr_aborted = 1;
        end
      end

  initial f_m_ackd = 0;
  generate
    for (N = 0; N < NM; N = N + 1) begin : GEN_FM_ACKD

      always @(posedge i_clk)
        if (i_reset) f_m_ackd[N] <= 0;
        else if (o_mack[N]) f_m_ackd[N] <= 1'b1;

    end
  endgenerate

  always @(posedge i_clk) cover (!f_cvr_aborted && (&f_m_ackd));

  generate
    if (NM > 1) begin

      always @(posedge i_clk) cover (!f_cvr_aborted && (&f_m_ackd[1:0]));
  ...
  initial f_s_ackd = 0;
  generate
    for (M = 0; M < NS; M = M + 1) begin : GEN_FS_ACKD

      always @(posedge i_clk)
        if (i_reset) f_s_ackd[M] <= 1'b0;
        else if (sgrant[M] && o_mack[sindex[M]]) f_s_ackd[M] <= 1'b1;

    end
  endgenerate

  always @(posedge i_clk) cover (!f_cvr_aborted && (&f_s_ackd[NS-1:0]));
