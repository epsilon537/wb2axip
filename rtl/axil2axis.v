...
module axil2axis #(
  ...
  ADDR_STATS = 2'b10, ADDR_FIFO = 2'b11;
  ...
  // {{{
  generate
    if ((OPT_TIMEOUT > 1) && OPT_SOURCE) begin
...
`ifdef FORMAL
      always @(*) assert (write_timer <= OPT_TIMEOUT - 1);
      always @(*) assert (write_timeout == (write_timer == 0));
`endif
    end else begin

      always @(*) write_timeout <= 1'b1;
  ...
  initial axil_berr = 0;
  always @(posedge S_AXI_ACLK)
    if (OPT_LOWPOWER && i_reset) axil_berr <= 0;
    else if (axil_write_ready) axil_berr <= (wfifo_full) && (awskd_addr[1] == ADDR_SOURCE[1]);
    else if (OPT_LOWPOWER && S_AXI_BREADY) axil_berr <= 1'b0;
  ...
  generate
    if (OPT_SOURCE) begin
      ...
      sfifo #(
          ...
          .i_wr(wfifo_write),
          .i_data({awskd_addr[0] == ADDR_SOURCE[0], wskd_data[SW-1:0]}),
          .o_full(wfifo_full),
          .o_fill(wfifo_fill),
          .i_rd(M_AXIS_TREADY),
      ...
      always @(*) M_AXIS_TVALID = !wfifo_empty;
  ...
  generate
    if (OPT_SINK) begin
      ...
      always @(*) S_AXIS_TREADY = !rfifo_full;
      assign read_rfifo = (axil_read_ready && arskd_addr == ADDR_SINK) && !rfifo_empty;

    end else begin

      always @(*) S_AXIS_TREADY = 1'b1;
  ...
  // {{{
  generate
    if (OPT_SINK && OPT_TIMEOUT > 1) begin
    ...
    begin
          ...
          if (read_timer > 0) read_timer <= read_timer - 1;
          read_timeout <= (read_timer <= 1);
        end

`ifdef FORMAL
      always @(*) assert (read_timer <= OPT_TIMEOUT - 1);
      always @(*) assert (read_timeout == (read_timer == 0));
`endif
    end else begin

      always @(*) read_timeout = 1'b1;
  ...
  always @(posedge S_AXI_ACLK)
    if (OPT_LOWPOWER && !S_AXI_ARESETN) axil_rerr <= 1'b0;
    else if (axil_read_ready) axil_rerr <= rfifo_empty && (arskd_addr[1] == ADDR_SINK[1]);
    else if (OPT_LOWPOWER && S_AXI_RREADY) axil_rerr <= 1'b0;
  ...
  always @(posedge S_AXI_ACLK)
    if (!S_AXI_ARESETN) begin
      ...
      // }}}
    end else if (!OPT_SINK) begin
      // {{{
      reads_completed <= reads_completed + (S_AXIS_TVALID ? 1 : 0);
      read_bursts_completed <= read_bursts_completed + ((S_AXIS_TVALID && S_AXIS_TLAST) ? 1 : 0);
      // }}}
    end else if (read_rfifo && !rfifo_empty) begin
      // {{{
    ...
    if (OPT_SOURCE) begin
      ...
      always @(posedge S_AXI_ACLK)
        if (!S_AXI_ARESETN) begin
          ...
          write_bursts_completed <= write_bursts_completed + (M_AXIS_TLAST ? 1 : 0);
        end
      // }}}
      ...
      casez (arskd_addr)
        {
          ...
          if (OPT_SIGN_EXTEND && rfifo_data[SW-1]) axil_read_data <= -1;
...
`ifdef FORMAL
  ...
  always @(posedge S_AXI_ACLK)
    if (!S_AXI_ARESETN) begin
      f_reads <= 0;
      f_read_pkts <= 0;
    end else if (OPT_SINK && (axil_read_ready && arskd_addr == ADDR_SINK && !rfifo_empty)) begin
      f_reads <= f_reads + 1;
      f_read_pkts <= f_read_pkts + (rfifo_last ? 1 : 0);
    end else if (!OPT_SINK && S_AXIS_TVALID) begin
      f_reads <= f_reads + 1;
      f_read_pkts <= f_read_pkts + (S_AXIS_TLAST ? 1 : 0);
    end

  always @(*) assert (f_reads == reads_completed);
  always @(*) assert (f_read_pkts == read_bursts_completed);

  always @(*) if (!OPT_SINK) assert (S_AXIS_TREADY);
  ...
  always @(posedge S_AXI_ACLK)
    if (!S_AXI_ARESETN) begin
      f_writes <= 0;
      f_write_pkts <= 0;
    end else if (OPT_SOURCE && M_AXIS_TVALID && M_AXIS_TREADY) begin
      f_writes <= f_writes + 1;
  ...
  always @(*) cover (S_AXI_ARESETN && writes_completed == 16);

  always @(*) cover (S_AXI_ARESETN && reads_completed == 16);

  always @(*) cover (S_AXI_ARESETN && writes_completed == 16 && reads_completed == 16);

  always @(*) cover (S_AXI_BVALID && S_AXI_BRESP != 2'b00);

  always @(*) cover (S_AXI_RVALID && S_AXI_RRESP != 2'b00);
