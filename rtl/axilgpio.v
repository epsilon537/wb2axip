...
module axilgpio #(
  ...
  // ADDR_CLEAR  = 3'b010,
  // ADDR_TOGGLE = 3'b011,
  ADDR_INDATA = 3'b100, ADDR_CHANGED = 3'b101, ADDR_MASK = 3'b110, ADDR_INT = 3'b111;
  ...
  always @(posedge S_AXI_ACLK) begin
    if (axil_write_ready) begin
      case ({
        (awskd_addr[2] && (NIN > 0)), awskd_addr[1:0]
      })
        3'b000: r_gpio <= wskd_gpio;
        3'b001: begin  // SET
          // {{{
          if (wskd_strb[0]) r_gpio[7:0] <= r_gpio[7:0] | wskd_data[7:0];
          if (wskd_strb[1]) r_gpio[15:8] <= r_gpio[15:8] | wskd_data[15:8];
          if (wskd_strb[2]) r_gpio[23:16] <= r_gpio[23:16] | wskd_data[23:16];
          if (wskd_strb[3]) r_gpio[31:24] <= r_gpio[31:24] | wskd_data[31:24];
        end
        // }}}
        3'b010: begin  // CLEAR
          // {{{
    ...
    if (i_reset) r_gpio[NOUT-1:0] <= DEFAULT_OUTPUT;
    if (NOUT < 32) r_gpio[31:((NOUT<32)?NOUT : 31)] <= 0;
  end
  ...
  // {{{
  generate
    if (NIN > 0) begin : INPUT_HANDLING
      // {{{
      // Poss interrupts: Toggle, Rise, Fall
      //  Only toggle is implemented here
      reg [NIN-1:0] last_gpio, qq_gpio, q_gpio, toggled, r_mask;
      ...
      always @(posedge S_AXI_ACLK)
        if (i_reset) {last_gpio, qq_gpio, q_gpio} <= 0;
        else {last_gpio, qq_gpio, q_gpio} <= {qq_gpio, q_gpio, i_gpio};
      // }}}
      ...
      initial toggled = 0;
      always @(posedge S_AXI_ACLK)
        if (i_reset) toggled <= 0;
        else begin
          for (ik = 0; ik < NIN; ik = ik + 1) begin
            if (axil_write_ready && awskd_addr == 3'b101 && wskd_strb[ik/8] && wskd_data[ik])
              toggled[ik] <= 1'b0;
            if (last_gpio[ik] ^ qq_gpio[ik]) toggled[ik] <= 1'b1;
          end
        end
      // }}}
      ...
      always @(posedge S_AXI_ACLK)
        if (i_reset) r_mask <= 0;
        else if (axil_write_ready && awskd_addr == 3'b110) r_mask <= wstrb_mask[NIN-1:0];
      // }}}
  ...
  always @(posedge S_AXI_ACLK)
    if (OPT_LOWPOWER && !S_AXI_ARESETN) axil_read_data <= 0;
      ...
      casez ({
        ((NIN > 0) && arskd_addr[2]), arskd_addr[1:0]
      })
...
`ifdef CHECK_REGISTERS
  ...
  always @(*) assume (f_obit < NOUT);

  // Verify o_gpio
  // {{{
  always @(posedge S_AXI_ACLK)
    if ($past(i_reset) || i_reset) begin
      if ($past(i_reset)) assert (o_gpio[f_obit] == DEFAULT_OUTPUT[f_obit]);
    end else if ($past(axil_write_ready && wskd_strb[f_obit/8])) begin
      case ($past(
          awskd_addr[2:0]
      ))
  ...
  generate
    if (NIN > 0) begin : CHECK_INPUT
      (* anyconst *) reg [$clog2(NIN)-1:0] f_ibit;

      always @(*) assume (f_ibit < NIN);

      always @(posedge S_AXI_ACLK)
        if ($past(i_reset) || i_reset) begin
          if ($past(i_reset)) assert (!ck_gpio[f_ibit]);
        end else begin
          if (!$past(i_reset, 2) && $past(ck_gpio[f_ibit]) != $past(ck_gpio[f_ibit], 2))
...
`endif
  ...
  always @(posedge S_AXI_ACLK) if (!i_reset) cover (o_int);

  always @(posedge S_AXI_ACLK) if (!i_reset) cover ($fell(o_int));
