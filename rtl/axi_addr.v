...
module axi_addr #(
  ...
  always @(*) begin
    ...
    if (i_burst != 0) begin
      ...
      // {{{
      if (DSZ == 0) increment = 1;
      else if (DSZ == 1) increment = (i_size[0]) ? 2 : 1;
      else if (DSZ == 2) increment = (i_size[1]) ? 4 : ((i_size[0]) ? 2 : 1);
      else if (DSZ == 3)
        case (i_size[1:0])
          2'b00: increment = 1;
          2'b01: increment = 2;
  ...
  always @(*) begin
    ...
    */
    ...
    //
    if (DSZ < 2) wrap_mask = wrap_mask | ({{(AW - 4) {1'b0}}, i_len[3:0]} << (i_size[0]));
    else if (DSZ < 4) wrap_mask = wrap_mask | ({{(AW - 4) {1'b0}}, i_len[3:0]} << (i_size[1:0]));
    else wrap_mask = wrap_mask | ({{(AW - 4) {1'b0}}, i_len[3:0]} << (i_size));

    if (AW > 12) wrap_mask[(AW-1):((AW>12)?12 : (AW-1))] = 0;
  end
  // }}}
  ...
  always @(*) begin
    ...
    // lower 12 bits
    if (AW > 12) o_next_addr[AW-1:((AW>12)?12 : (AW-1))] = i_last_addr[AW-1:((AW>12)?12 : (AW-1))];
    // }}}
  end
  // }}}
  ...
  wire unused;
  assign unused = (LENB <= 4) ? &{1'b0, i_len[0]} : &{1'b0, i_len[LENB-1:4], i_len[0]};
