...
module axi3reorder #(
  ...
  //
  generate
    if (OPT_METHOD == MTHD_NONE) begin : IGNORE_AW_CHANNEL
      ...
      always @(*) cid_valid = S_AXI3_AWVALID;
      always @(*) current_id = S_AXI3_AWID;
      ...
      always @(posedge S_AXI_ACLK)
        if (!S_AXI_ARESETN) cid_valid <= 1'b0;
        else if (read_awid_fifo) cid_valid <= !awfifo_empty;

      // The current ID is given by the write address channel
      always @(posedge S_AXI_ACLK) if (read_awid_fifo) current_id <= awfifo_id;
  ...
  //
  generate
    if (OPT_METHOD == MTHD_NONE) begin : COPY_SW_TO_MW  // Copy S_AXI3_W* values to M_AXIW*
      // {{{
      // {{{
      always @(*) begin
      ...
      // }}}
    end else if (OPT_METHOD == MTHD_SHIFT_REGISTER) begin : SHIFT_REGISTER
      // {{{
      ...
      for (gk = 0; gk < NSREG; gk = gk + 1) begin
        ...
        always @(*) begin
          sr_advance[gk] = 0;
          if ((gk > 0) && (sr_advance[gk-1])) sr_advance[gk] = 1;
          if ((!M_AXI_WVALID || M_AXI_WREADY) && cid_valid && current_id == sr_id[gk])
            sr_advance[gk] = 1;
          if (!sr_valid[gk]) sr_advance[gk] = 0;
        end
        // }}}
        ...
        always @(*) begin
          ...
          if (sr_valid[gk] && (!sr_advance[gk] || (gk < NSREG - 1 && sr_valid[gk+1])))
            sr_write[gk] = 0;
          if (gk > 0 && (!sr_valid[gk-1] || (sr_advance[gk-1] && !sr_valid[gk]))) sr_write[gk] = 0;
        end
        // }}}
        ...
        always @(posedge S_AXI_ACLK)
          if (!S_AXI_ARESETN) sr_valid[gk] <= 0;
          else if (sr_write[gk]) sr_valid[gk] <= 1;
          else if (sr_advance[gk] && gk < NSREG - 1) sr_valid[gk] <= sr_valid[gk+1];
          else if (sr_advance[gk]) sr_valid[gk] <= 0;
        // }}}
        ...
        always @(posedge S_AXI_ACLK)
          ...
          end else if ((!OPT_LOWPOWER && !sr_valid[gk]) || sr_write[gk]) begin
            sr_id[gk]   <= S_AXI3_WID;
            sr_data[gk] <= S_AXI3_WDATA;
            sr_strb[gk] <= S_AXI3_WSTRB;
            sr_last[gk] <= S_AXI3_WLAST;
      ...
      always @(*) begin
        ...
        if (!known_next && current_id == sr_id[ik]) begin
          sr_next = ik[LGWFIFO-1:0];
          known_next = sr_valid[ik];
        end

        if (!cid_valid) known_next = 0;
      end
      ...
      always @(*) read_beat_fifo = (!M_AXI_WVALID || M_AXI_WREADY) && (known_next);

      initial M_AXI_WVALID = 0;
      always @(posedge S_AXI_ACLK)
        if (!S_AXI_ARESETN) M_AXI_WVALID <= 0;
        else if (!M_AXI_WVALID || M_AXI_WREADY) M_AXI_WVALID <= known_next;
      ...
      always @(posedge S_AXI_ACLK)
        if (OPT_LOWPOWER && !S_AXI_ARESETN) begin
          ...
          if (OPT_LOWPOWER && !known_next) begin
            // {{{
            M_AXI_WID   <= 0;
            M_AXI_WDATA <= 0;
            M_AXI_WSTRB <= 0;
...
`ifdef FORMAL
      always @(*)
        ...
        end else if (S_AXI_ARESETN) assert (sr_write == 0);

      always @(*) assert (((sr_write << 1) & sr_valid) == 0);

      reg cvr_sreg_full;

      initial cvr_sreg_full = 0;
      always @(posedge S_AXI_ACLK)
        if (!S_AXI_ARESETN) cvr_sreg_full <= 0;
        else if (!S_AXI3_WREADY) cvr_sreg_full <= 1;

      always @(*) cover (cvr_sreg_full && sr_valid == 0);

`endif
      // }}}
    end else if (OPT_METHOD == MTHD_PERID_FIFOS) begin : MULTIPLE_FIFOS
      // {{{
      ...
      //
      for (gk = 0; gk < NUM_FIFOS; gk = gk + 1) begin
        // {{{
        ...
        sfifo #(
            ...
            .i_data({S_AXI3_WDATA, S_AXI3_WSTRB, S_AXI3_WLAST}),
            .o_full(wbfifo_full[gk]),
            .o_fill(wbfifo_fill),
            .i_rd(read_beat_fifo && current_id == gk),
            .o_data({wbfifo_data[gk], wbfifo_strb[gk], wbfifo_last[gk]}),
      ...
      always @(posedge S_AXI_ACLK)
        if (!S_AXI_ARESETN) M_AXI_WVALID <= 0;
        else if (!M_AXI_WVALID || M_AXI_WREADY) M_AXI_WVALID <= read_beat_fifo;
      ...
      always @(posedge S_AXI_ACLK)
        if (OPT_LOWPOWER && !S_AXI_ARESETN) begin
          ...
          if (OPT_LOWPOWER && !read_beat_fifo) begin
            // {{{
            M_AXI_WID   <= 0;
            M_AXI_WDATA <= 0;
            M_AXI_WSTRB <= 0;
...
`ifdef FORMAL
  ...
  always @(posedge S_AXI_ACLK)
    if (!f_past_valid || $past(!S_AXI_ARESETN)) begin
      ...
      if ($past(S_AXI3_AWVALID && !S_AXI3_AWREADY)) begin
        assume (S_AXI3_AWVALID);
        assume ($stable(S_AXI3_AWID));
      end

      if ($past(S_AXI3_WVALID && !S_AXI3_WREADY)) begin
        assume (S_AXI3_WVALID);
      ...
      if ($past(M_AXI_WVALID && !M_AXI_WREADY)) begin
        assert (M_AXI_WVALID);
  ...
  generate
    if (OPT_METHOD != MTHD_NONE) begin : F_FIFO_CHECK
      ...
      sfifo #(
          ...
          .i_clk(S_AXI_ACLK),
          .i_reset(!S_AXI_ARESETN),
          .i_wr(S_AXI3_WVALID && S_AXI3_WREADY && S_AXI3_WID == f_const_id),
          .i_data({S_AXI3_WID, S_AXI3_WDATA, S_AXI3_WSTRB, S_AXI3_WLAST}),
          .o_full(f_ckfifo_full),
          .o_fill(f_ckfifo_fill),
          .i_rd(M_AXI_WVALID && M_AXI_WREADY && M_AXI_WID == f_const_id),
          .o_data({f_ckfifo_id, f_ckfifo_data, f_ckfifo_strb, f_ckfifo_last}),
          .o_empty(f_ckfifo_empty)
      );

      always @(*)
        if (S_AXI_ARESETN && M_AXI_WVALID && M_AXI_WID == f_const_id) begin
  ...
  always @(posedge S_AXI_ACLK)
    if (!S_AXI_ARESETN) f_awid_count = 0;
    else
      case ({
        S_AXI3_AWVALID && S_AXI3_AWREADY && S_AXI3_AWID == f_const_id,
        M_AXI_WVALID && M_AXI_WREADY && M_AXI_WLAST && M_AXI_WID == f_const_id
      })
  ...
  always @(*) if (M_AXI_WVALID && M_AXI_WID == f_const_id) assert (f_awid_count > 0);

  generate
    if (OPT_LOWPOWER) begin : F_LOWPOWER_CHECK
      always @(*) if (!S_AXI3_AWVALID) assume (S_AXI3_AWID == 0);
