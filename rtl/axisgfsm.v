...
module axisgfsm #(
  ...
  localparam [C_AXI_ADDR_WIDTH-1:0] TBL_SIZE = (C_AXI_ADDR_WIDTH <= 30) ? (4 * 3) : (4 * 5);
  ...
  reg [2:0] sgstate;
  reg dma_err, dma_abort, dma_done, dma_busy, dma_starting, dma_aborting;
  reg [59:0] r_pf_pc;
  ...
  always @(*) begin
    ...
    if (!o_busy || o_pf_clear_cache || o_new_pc) o_pf_ready = 0;
  end
  // }}}
  ...
  always @(*) begin
    ...
    if (sgstate == SG_WAIT && i_dma_complete && (tbl_last || tbl_int_enable)) o_int = 1'b1;
    if (i_pf_valid && o_pf_ready && i_pf_illegal) o_int = 1'b1;
  end
  // }}}
  ...
  // {{{
  always @(*)
    if (dma_busy || dma_starting || (o_dmac_wvalid && !i_dmac_wready)) dma_op_complete = 0;
    else dma_op_complete = (!o_dmac_wvalid || !o_dmac_wstrb[0]);
  // }}}
  ...
  always @(*) begin
    dma_terminate = 0;
    if (i_abort || tbl_last || dma_aborting) dma_terminate = 1;
    if (dma_err || i_pf_illegal) dma_terminate = 1;
  end
  // }}}
  ...
  always @(posedge S_AXI_ACLK) begin
    ...
    if (i_dmac_wready) o_dmac_wvalid <= 1'b0;
    if (!o_dmac_wvalid || i_dmac_wready) o_dmac_wstrb <= 4'hf;
    o_done <= 1'b0;
    o_err  <= 1'b0;
    // }}}
    ...
    case (sgstate)
      ...
      begin
        ...
        if (i_start) begin
          o_new_pc <= 1'b1;
          if (C_AXI_ADDR_WIDTH > 30) sgstate <= SG_SRCHALF;
          else sgstate <= SG_SRCADDR;
          o_busy <= 1'b1;
        end else begin
          o_new_pc <= 1'b0;
      ...
      // {{{
      if (i_pf_valid && (!o_dmac_wvalid || i_dmac_wready)) begin
        o_dmac_wvalid <= 1'b1;
      ...
      // {{{
      if (i_pf_valid && o_pf_ready) begin
        o_dmac_wvalid <= !i_pf_insn[31];
        o_dmac_waddr  <= (C_AXI_ADDR_WIDTH <= 30) ? DMA_SRCLO : DMA_SRCHI;
        o_dmac_wdata  <= i_pf_insn;
        // r_pf_pc[31:0] <= i_pf_insn;
        if (C_AXI_ADDR_WIDTH <= 30) begin
          sgstate <= SG_DSTADDR;
          o_tbl_addr <= i_pf_pc;
        ...
        case (i_pf_insn[31:30])
          ...
          2'b10: begin  // Skip
            ...
            o_new_pc <= 1'b1;
            r_pf_pc[C_AXI_ADDR_WIDTH-1:0] <= r_pf_pc[C_AXI_ADDR_WIDTH-1:0] + TBL_SIZE;
          end
          2'b11: begin  // Jump
            o_new_pc <= 1'b1;
      ...
      // {{{
      if (!o_dmac_wvalid || i_dmac_wready) begin
        o_dmac_wvalid <= 1'b1;
      ...
      // {{{
      if (dma_op_complete) begin
        // o_int <= int_enable;
        r_pf_pc[C_AXI_ADDR_WIDTH-1:0] <= o_tbl_addr;
        if (dma_terminate) begin
          o_pf_clear_cache <= 1'b1;
          sgstate <= SG_IDLE;
          o_busy <= 1'b0;
          o_done <= 1'b1;
          o_err <= dma_err || dma_abort;
          if (i_pf_illegal) r_pf_pc[C_AXI_ADDR_WIDTH-1:0] <= i_pf_pc;
    ...
    if (i_abort && (!o_dmac_wvalid || i_dmac_wready)) begin
      // {{{
      o_dmac_wstrb <= 4'h8;
      o_dmac_wdata[31:24] <= ABORT_KEY;
      o_dmac_wvalid <= o_dmac_wstrb[0] && dma_busy && !dma_err && (sgstate == SG_WAIT);
      // }}}
      if (!dma_busy && (sgstate != SG_WAIT)) begin
        sgstate <= SG_IDLE;
        o_done <= 1'b1;
        o_new_pc <= 1'b0;
        o_busy <= 1'b0;
  ...
  always @(posedge S_AXI_ACLK)
    if (!S_AXI_ARESETN) dma_starting <= 0;
    else if (sgstate != SG_WAIT) dma_starting <= 0;
    else if (!o_dmac_wvalid || o_dmac_waddr != DMA_CONTROL) dma_starting <= 0;
    else dma_starting <= o_dmac_wdata[0] && o_dmac_wstrb[0];

  always @(*) o_pf_pc = {r_pf_pc[C_AXI_ADDR_WIDTH-1:2], 2'b00};
  // }}}
  ...
  always @(posedge S_AXI_ACLK)
    if (!S_AXI_ARESETN) dma_aborting <= 0;
    else if (sgstate != SG_WAIT) dma_aborting <= 0;
    else if (!dma_aborting) begin
      if (i_abort) dma_aborting <= 1;
...
`ifdef FORMAL
  always @(*)
    if (S_AXI_ARESETN) begin
      if (o_dmac_wvalid && !o_dmac_wstrb[0]) assert (dma_aborting);
      if (sgstate != SG_WAIT && sgstate != SG_IDLE) assert (!dma_aborting);
    end
`endif
  // }}}
  // }}}
...
`ifdef FORMAL
  ...
  reg f_tbl_last, f_tbl_skip, f_tbl_jump, f_tbl_int_enable;
  ...
  always @(*) if (o_new_pc) assert (o_pf_pc[1:0] == 2'b00);

  always @(posedge S_AXI_ACLK)
    if (o_new_pc) f_pc <= o_pf_pc;
    else if (i_pf_valid && o_pf_ready) f_pc <= f_pc + 4;

  always @(*) if (i_pf_valid) assume (i_pf_pc == f_pc);

  always @(*) if (S_AXI_ARESETN && !o_new_pc && !o_pf_clear_cache) assert (f_pc[1:0] == 2'b00);

  always @(posedge S_AXI_ACLK)
    if (!f_past_valid || $past(!S_AXI_ARESETN || o_new_pc || o_pf_clear_cache)) begin
      assume (!i_pf_illegal);
    end else if (!$rose(i_pf_valid)) begin
      assume (!$rose(i_pf_illegal));
    end else assume ($stable(i_pf_illegal));
  // }}}
  ////////////////////////////////////////////////////////////////////////
  //
  ...
  //
  //
  reg [F_LGDEPTH-1:0] fdma_rd_outstanding, fdma_wr_outstanding, fdma_awr_outstanding;
  ...
  always @(posedge S_AXI_ACLK)
    if (!S_AXI_ARESETN) f_dma_arvalid <= 1'b0;
    else f_dma_arvalid <= 1'b1;

  always @(*) f_dma_arready = 1'b1;
  ...
  assert property (@(posedge S_AXI_ACLK) !S_AXI_ARESETN |=> sgstate == SG_IDLE && !o_dmac_wvalid);
  ...
  assert property (@(posedge S_AXI_ACLK) fdma_wr_outstanding == fdma_awr_outstanding);

  assert property (@(posedge S_AXI_ACLK) fdma_wr_outstanding == (f_dma_bvalid ? 1 : 0));

  initial f_dma_bvalid = 1'b0;
  always @(posedge S_AXI_ACLK)
    if (!S_AXI_ARESETN) f_dma_bvalid <= 0;
    else if (o_dmac_wvalid && i_dmac_wready) f_dma_bvalid <= 1;
    else f_dma_bvalid <= 0;

  initial f_dma_rvalid = 1'b0;
  always @(posedge S_AXI_ACLK)
    if (!S_AXI_ARESETN) f_dma_rvalid <= 0;
    else if (f_dma_arvalid) f_dma_rvalid <= 1;
    else f_dma_rvalid <= 0;
  // }}}
  ////////////////////////////////////////////////////////////////////////
  //
  ...
  always @(posedge S_AXI_ACLK)
    if (!S_AXI_ARESETN) f_dma_busy <= 0;
    else if (o_dmac_wvalid && i_dmac_wready && o_dmac_waddr == 0
      && o_dmac_wstrb[0] && o_dmac_wdata[0])
      f_dma_busy <= 1'b1;
    else if (f_dma_busy) f_dma_busy <= !f_dma_complete;

  always @(*)
    if (S_AXI_ARESETN) begin
      if (sgstate != SG_WAIT)
        assert (!f_dma_busy);
        else if (o_dmac_wvalid && !dma_aborting) assert (!f_dma_busy);
    end
  ...
  always @(*) if (!f_dma_busy) assume (!f_dma_complete);

  assume property (@(posedge S_AXI_ACLK)
    disable iff (!S_AXI_ARESETN)
    !dma_busy ##1 !dma_busy |-> !$rose(
      dma_err
  ));

  assume property (@(posedge S_AXI_ACLK) disable iff (!S_AXI_ARESETN) $rose(dma_busy) |=> !dma_err);
  ...
  always @(*)
    if (o_new_pc) begin
      ...
      end else assert (sgstate == SG_SRCHALF);
    end
  ...
  always @(posedge S_AXI_ACLK)
    if (S_AXI_ARESETN && (sgstate != SG_IDLE)) begin
      if ($stable(sgstate)) begin
        assume ($stable(f_tblentry));
      end else if ((C_AXI_ADDR_WIDTH > 30) && (sgstate != SG_SRCHALF)) begin
        assume ($stable(f_tblentry));
  ...
  generate
    if (C_AXI_ADDR_WIDTH <= 30) begin : SMALL_ADDRESS_SPACE
      // {{{
      reg [5:0] f_dma_seq;
      ...
      always @(*)
        if (i_pf_valid)
          case (sgstate)
            ...
            // SG_WAIT:  begin end
            SG_SRCADDR: begin
              assume (i_pf_insn == f_tblentry[31:0] && (i_pf_pc == f_tbladdr));
            end
            SG_DSTADDR: begin
              assume (i_pf_insn == f_tblentry[63:32] && (i_pf_pc == f_tbladdr + 4));
            end
            SG_LENGTH: begin
              assume (i_pf_insn == f_tblentry[95:64] && (i_pf_pc == f_tbladdr + 8));
            end
            default: begin
            end
          endcase
      ...
      initial f_dma_seq = 0;
      always @(posedge S_AXI_ACLK)
        if (!S_AXI_ARESETN) f_dma_seq <= 0;
        else begin

          // SG_SRCADDR, or entering SG_SRCADDR
          // {{{
          if ((f_dma_seq == 0) && (i_start)) f_dma_seq <= 1;

          if (f_dma_seq[0] || o_new_pc) begin
            assert (sgstate == SG_SRCADDR);
            ...
            if (i_pf_valid && o_pf_ready) begin
              ...
              if (f_tbl_jump || f_tbl_skip) f_dma_seq <= 1;
            end
            ...
            if ($past(sgstate == SG_SRCADDR && i_pf_valid && o_pf_ready)) begin
              // Jump or skip

              assert (o_new_pc);
              // Check the jump address
              // if ($past(i_pf_insn[31:30] == 2'b11))
              if ($past(f_tbl_jump)) begin
          ...
          if (f_dma_seq[1]) begin
            ...
            f_dma_seq <= 2;
            if (i_pf_valid && o_pf_ready) f_dma_seq <= 4;
          end
          // }}}

          // SG_LENGTH
          // {{{
          if (f_dma_seq[2]) begin
            assert (sgstate == SG_LENGTH);
            assert (tbl_last == f_tbl_last);
            assert (tbl_int_enable == f_tbl_int_enable);
            assert (o_tbl_addr == f_tbladdr);
            if ($rose(f_dma_seq[2])) assert (o_dmac_wvalid);
            ...
            if (i_pf_valid && o_pf_ready) begin
              if (i_pf_insn == 0) begin
                if (tbl_last) f_dma_seq <= 0;
                else f_dma_seq <= 1;
              end else f_dma_seq <= 8;
            end
          end
          // }}}
          ...
          if (f_dma_seq[3]) begin
            ...
            // assert(f_pc == o_pf_pc);
            f_dma_seq <= 8;
            if (i_dmac_wready) f_dma_seq <= 16;
          end
          // }}}

          // SG_WAIT && o_dmac_wvalid
          // {{{
          if (f_dma_seq[4]) begin
            ...
            // assert(f_pc == o_pf_pc);
            f_dma_seq <= 16;
            if (o_dmac_wvalid && i_dmac_wready) f_dma_seq <= 32;
          end
          // }}}

          // SG_WAIT && !o_dmac_wvalid
          // {{{
          if (f_dma_seq[5]) begin
            assert (sgstate == SG_WAIT);
            assert (tbl_last == f_tbl_last);
            assert (o_tbl_addr == f_tbladdr + TBL_SIZE);
            assert (o_dmac_waddr == DMA_CONTROL);
            // assert(o_dmac_wdata == f_tblentry[95:64]);
            if (f_never_abort) begin
              assert (&o_dmac_wstrb);
              assert (!o_dmac_wvalid);
            end else assert (o_dmac_wstrb == 4'h8 || o_dmac_wstrb == 4'hf);
            if (o_dmac_wvalid) assert (!o_dmac_wstrb[0]);
            f_dma_seq <= 32;
            assert (o_pf_pc == f_tbladdr);
            assert (f_pc == f_tbladdr + TBL_SIZE);
            // assert(f_pc == o_pf_pc);
          ...
          // pf_illegal
          // {{{
          if ((|f_dma_seq[2:0]) && i_pf_illegal) f_dma_seq <= 0;
          // }}}

          // i_abort
          if ((|f_dma_seq[3:0]) && i_abort && (!o_dmac_wvalid || i_dmac_wready)) f_dma_seq <= 0;
        end

      always @(*)
        if (f_dma_seq == 0) begin
      ...
      always @(posedge S_AXI_ACLK)
        if (!f_past_valid || $past(!S_AXI_ARESETN)) begin
        end else if (sgstate == 0) begin
      ...
      cover property (@(posedge S_AXI_ACLK) f_dma_seq[5] ##2 f_dma_seq[0]);  // !!!

      cover property (@(posedge S_AXI_ACLK) f_dma_seq[5] && tbl_last && i_dma_complete);  // !!!

      cover property (@(posedge S_AXI_ACLK) f_dma_seq[5] && i_dma_complete);  // !!!
      // }}}
    end else begin : BIG_ADDR
    end
  endgenerate

  always @(*) assert (o_busy == (sgstate != SG_IDLE));
  ...
  assert property (@(posedge S_AXI_ACLK) !o_done && !o_busy |-> !o_err);

  always @(*) if (o_pf_ready) assert (!o_dmac_wvalid || i_dmac_wready);
  ...
  always @(*) if (f_never_abort) assume (!i_abort);
