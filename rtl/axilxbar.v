...
`endif
  ...
  generate
    for (N = 0; N < NM; N = N + 1) begin : DECODE_WRITE_REQUEST
      ...
      always @(*) begin
        ...
        if (!swgrant[N]) slave_awaccepts[N] = 1'b0;
        if (swfull[N]) slave_awaccepts[N] = 1'b0;
        if (!wrequest[N][swindex[N]]) slave_awaccepts[N] = 1'b0;
      ...
      always @(*) begin
        ...
        if (!swgrant[N]) slave_waccepts[N] = 1'b0;
        if (!wdata_expected[N]) slave_waccepts[N] = 1'b0;
        if (!wgrant[N][NS] && (m_axi_wvalid[swindex[N]] && !m_axi_wready[swindex[N]]))
          slave_waccepts[N] = 1'b0;
        if (wgrant[N][NS] && (S_AXI_BVALID[N] && !S_AXI_BREADY[N])) slave_waccepts[N] = 1'b0;
      end
      // }}}
      ...
      always @(*) begin
        ...
        wrequest[N] = 0;
        if (!swfull[N]) wrequest[N][NS:0] = wdecode;
      end
  ...
  generate
    for (N = 0; N < NM; N = N + 1) begin : DECODE_READ_REQUEST
      ...
      always @(*) begin
        ...
        rrequest[N] = 0;
        if (!srfull[N]) rrequest[N][NS:0] = rdecode;
      end
      ...
      always @(*) begin
        ...
        if (!srgrant[N]) slave_raccepts[N] = 1'b0;
        if (srfull[N]) slave_raccepts[N] = 1'b0;
        // verilator lint_off  WIDTH
        if (!rrequest[N][srindex[N]]) slave_raccepts[N] = 1'b0;
        // verilator lint_on  WIDTH
        if (!rgrant[N][NS]) begin
          if (m_axi_arvalid[srindex[N]] && !m_axi_arready[srindex[N]]) slave_raccepts[N] = 1'b0;
        end else if (S_AXI_RVALID[N] && !S_AXI_RREADY[N]) slave_raccepts[N] = 1'b0;
      end
      // }}}
  ...
  always @(*) begin : DECONFLICT_WRITE_REQUESTS
    ...
    for (iN = 1; iN < NM; iN = iN + 1) wrequested[iN] = 0;
    ...
    for (iM = 0; iM < NS; iM = iM + 1) begin
      ...
      for (iN = 1; iN < NM; iN = iN + 1) begin
        ...
        if (wrequest[iN-1][iM] && (!swgrant[iN-1] || swempty[iN-1])) wrequested[iN][iM] = 1;
  ...
  // rrequested
  // {{{
  always @(*) begin : DECONFLICT_READ_REQUESTS
    ...
    for (iM = 0; iM < NS; iM = iM + 1) begin
      ...
      for (iN = 1; iN < NM; iN = iN + 1) begin
        ...
        if (rrequest[iN-1][iM] && (!srgrant[iN-1] || srempty[iN-1])) rrequested[iN][iM] = 1;
        // Otherwise, if it's already claimed, then
        // it can't be claimed again
  ...
  generate
    for (M = 0; M < NS; M = M + 1) begin
      ...
      always @(*) begin
        mwgrant[M] = 0;
        for (iN = 0; iN < NM; iN = iN + 1) if (wgrant[iN][M]) mwgrant[M] = 1;
      end

      always @(*) begin
        mrgrant[M] = 0;
        for (iN = 0; iN < NM; iN = iN + 1) if (rgrant[iN][M]) mrgrant[M] = 1;
      end
      // }}}
    end
  endgenerate
  ...
  generate
    for (N = 0; N < NM; N = N + 1) begin : ARBITRATE_WRITE_REQUESTS
      ...
      always @(*) begin
        ...
        if (swgrant[N] && !swempty[N]) stay_on_channel = 1;
      end
      // }}}
...
`ifdef FORMAL
        // {{{
        always @(*) assert (linger == (linger_counter != 0));
        // }}}
`endif
        // }}}
      end
      ...
      always @(*) begin
        leave_channel = 0;
        if (!m_awvalid[N] && (!linger || wrequested[NM][swindex[N]]))
  ...
  generate
    for (N = 0; N < NM; N = N + 1) begin : ARBITRATE_READ_REQUESTS
      ...
      always @(*) begin
        ...
        if (srgrant[N] && !srempty[N]) stay_on_channel = 1;
      end
      // }}}
...
`endif
      ...
      always @(*) begin
        leave_channel = 0;
        if (!m_arvalid[N] && (!linger || rrequested[NM][srindex[N]]))
          // Leave the channel after OPT_LINGER counts
          // of the channel being idle, or when someone
          // else asks for the channel
          leave_channel = 1;
    ...
    for (M = 0; M < NS; M = M + 1) begin : SLAVE_WRITE_INDEX
      ...
      if (NM <= 1) begin
        ...
        always @(*) begin
          ...
          if ((!swgrant[iN] || swempty[iN]) && (wrequest[iN][M] && !wrequested[iN][M]))
    ...
    for (M = 0; M < NS; M = M + 1) begin : SLAVE_READ_INDEX
      ...
      if (NM <= 1) begin
        ...
        always @(*) begin
          resrindex = 0;
          for (iN = 0; iN < NM; iN = iN + 1)
          if ((!srgrant[iN] || srempty[iN]) && (rrequest[iN][M] && !rrequested[iN][M]))
  ...
  generate
    for (M = 0; M < NS; M = M + 1) begin : WRITE_SLAVE_OUTPUTS
      ...
      always @(posedge S_AXI_ACLK)
        if (!S_AXI_ARESETN || !mwgrant[M]) axi_awvalid <= 0;
      ...
      always @(posedge S_AXI_ACLK)
        if (!S_AXI_ARESETN || !mwgrant[M]) axi_wvalid <= 0;
      ...
      always @(posedge S_AXI_ACLK)
        if (!S_AXI_ARESETN || !mwgrant[M]) axi_bready <= 1;
        else if (!mbstall) axi_bready <= 1;
...
`endif
  ...
  generate
    for (M = 0; M < NS; M = M + 1) begin : READ_SLAVE_OUTPUTS
      ...
      always @(posedge S_AXI_ACLK)
        if (!S_AXI_ARESETN || !mrgrant[M]) axi_arvalid <= 0;
        else if (!arstall) begin
      ...
      always @(posedge S_AXI_ACLK)
        if (!S_AXI_ARESETN || !mrgrant[M]) axi_rready <= 1;
        else if (!srstall) axi_rready <= 1;
...
`endif
  ...
  generate
    for (N = 0; N < NM; N = N + 1) begin : WRITE_RETURN_CHANNEL
      ...
      always @(*)
        if (wgrant[N][NS]) i_axi_bvalid = m_wvalid[N] && slave_waccepts[N];
        else i_axi_bvalid = m_axi_bvalid[swindex[N]];
      ...
      initial r_bresp[N] = 0;
      always @(posedge S_AXI_ACLK)
        if (OPT_LOWPOWER && !S_AXI_ARESETN) r_bresp[N] <= 0;
        else if (OPT_LOWPOWER && (!swgrant[N] || S_AXI_BREADY[N])) r_bresp[N] <= 0;
        else if (!r_bvalid[N]) begin
          if (!OPT_LOWPOWER || (i_axi_bvalid && !wgrant[N][NS] && mbstall)) begin
            r_bresp[N] <= i_axi_bresp;
          end else r_bresp[N] <= 0;
        end
      // }}}
      ...
      always @(posedge S_AXI_ACLK)
        ...
        else if (!mbstall) axi_bvalid <= swgrant[N] && (r_bvalid[N] || i_axi_bvalid);
      // }}}
      ...
      initial axi_bresp = 0;
      always @(posedge S_AXI_ACLK)
        if (OPT_LOWPOWER && !S_AXI_ARESETN) axi_bresp <= 0;
        else if (OPT_LOWPOWER && !swgrant[N]) axi_bresp <= 0;
        else if (!mbstall) begin
          if (r_bvalid[N]) axi_bresp <= r_bresp[N];
          else if (!OPT_LOWPOWER || i_axi_bvalid) axi_bresp <= i_axi_bresp;
          else axi_bresp <= 0;

          if (wgrant[N][NS] && (!OPT_LOWPOWER || i_axi_bvalid)) axi_bresp <= INTERCONNECT_ERROR;
        end
      // }}}
...
`ifdef FORMAL
      ...
      always @(*)
        if (swgrant[N])
          ...
          else assert (!r_bvalid[N]);
      always @(*) if (OPT_LOWPOWER && !r_bvalid[N]) assert (r_bresp[N] == 0);

      always @(*) if (OPT_LOWPOWER && !axi_bvalid) assert (axi_bresp == 0);
      // }}}
`endif
      // }}}
    end
  ...
  generate
    for (N = 0; N < NM; N = N + 1) begin : READ_RETURN_CHANNEL
      ...
      always @(*)
        if (rgrant[N][NS]) i_axi_rvalid = m_arvalid[N] && slave_raccepts[N];
        else i_axi_rvalid = m_axi_rvalid[srindex[N]];

      assign srstall = S_AXI_RVALID[N] && !S_AXI_RREADY[N];

      initial r_rvalid[N] = 0;
      always @(posedge S_AXI_ACLK)
        if (!S_AXI_ARESETN) r_rvalid[N] <= 0;
        else if (srstall && !r_rvalid[N])
          r_rvalid[N] <= srgrant[N] && !rgrant[N][NS] && i_axi_rvalid;
        else if (!srstall) r_rvalid[N] <= 0;
      ...
      always @(posedge S_AXI_ACLK)
        ...
        else if (!srstall) axi_rvalid <= srgrant[N] && (r_rvalid[N] || i_axi_rvalid);
      ...
      always @(posedge S_AXI_ACLK)
        if (OPT_LOWPOWER && !S_AXI_ARESETN) begin
          ...
          if (r_rvalid[N]) begin
            ...
            if (rgrant[N][NS]) axi_rresp <= INTERCONNECT_ERROR;
          end else begin
            axi_rresp <= 0;
            axi_rdata <= 0;
          end
...
`ifdef FORMAL
      ...
      always @(*)
        if (srgrant[N] && !rgrant[N][NS])
          ...
          else assert (!r_rvalid[N]);
      always @(*)
        if (OPT_LOWPOWER && !r_rvalid[N]) begin
          assert (r_rresp[N] == 0);
          assert (r_rdata[N] == 0);
        end

      always @(*)
        if (OPT_LOWPOWER && !axi_rvalid) begin
          assert (axi_rresp == 0);
          assert (axi_rdata == 0);
        end
      // }}}
`endif
  ...
  generate
    for (N = 0; N < NM; N = N + 1) begin : COUNT_PENDING
      // {{{
      reg [LGMAXBURST-1:0] wpending, awpending, rpending, missing_wdata;
      ...
      always @(posedge S_AXI_ACLK)
        if (!S_AXI_ARESETN) begin
          ...
          case ({
            (m_awvalid[N] && slave_awaccepts[N]), (S_AXI_BVALID[N] && S_AXI_BREADY[N])
          })
            2'b01: begin
      ...
      initial wpending = 0;
      always @(posedge S_AXI_ACLK)
        if (!S_AXI_ARESETN) wpending <= 0;
        else
          case ({
            (m_wvalid[N] && slave_waccepts[N]), (S_AXI_BVALID[N] && S_AXI_BREADY[N])
          })
      ...
      always @(posedge S_AXI_ACLK)
        if (!S_AXI_ARESETN) missing_wdata <= 0;
        else begin
      ...
      always @(posedge S_AXI_ACLK)
        if (!S_AXI_ARESETN) r_wdata_expected <= 0;
        else
          case ({
            m_awvalid[N] && slave_awaccepts[N], m_wvalid[N] && slave_waccepts[N]
          })
      ...
      always @(posedge S_AXI_ACLK)
        if (!S_AXI_ARESETN) begin
          ...
          case ({
            (m_arvalid[N] && slave_raccepts[N]), (S_AXI_RVALID[N] && S_AXI_RREADY[N])
          })
            2'b01: begin
              rpending   <= rpending - 1;
...
`ifdef FORMAL
      ...
      always @(*) assert (awpending >= wpending);
      // }}}
`endif
      // }}}
    end
...
`endif
  ...
  generate
    for (N = 0; N < NM; N = N + 1) begin : CHECK_MASTER_GRANTS
      // {{{
  ...
  generate
    for (N = 0; N < NM; N = N + 1) begin : CHECK_MASTERS
      // {{{
      faxil_slave #(
          .C_AXI_DATA_WIDTH(DW),
          .C_AXI_ADDR_WIDTH(AW),
      ...
      always @(*)
        if (S_AXI_ARESETN)
          assert (fm_wr_outstanding[N] == {1'b0, w_swpending[N]} + (S_AXI_WREADY[N] ? 0 : 1));
      ...
      always @(*)
        if (S_AXI_ARESETN)
          assert (fm_wr_outstanding[N] >= (S_AXI_WREADY[N] ? 0 : 1) + (S_AXI_BVALID[N] ? 1 : 0));
      ...
      always @(*)
        if (S_AXI_ARESETN && !swgrant[N]) begin
      ...
      always @(*)
        if (S_AXI_ARESETN)
          assert (fm_rd_outstanding[N] >= (S_AXI_ARREADY[N] ? 0 : 1) + (S_AXI_RVALID[N] ? 1 : 0));
      ...
      always @(*)
        if (S_AXI_ARESETN && !srgrant[N]) begin
  ...
  generate
    for (M = 0; M < NS; M = M + 1) begin : CHECK_SLAVES
      // {{{
      faxil_master #(
          .C_AXI_DATA_WIDTH(DW),
          .C_AXI_ADDR_WIDTH(AW),
      ...
      always @(*)
        if (M_AXI_AWVALID[M])
          assert (((M_AXI_AWADDR[M*AW+:AW] ^ SLAVE_ADDR[M*AW+:AW]) & SLAVE_MASK[M*AW+:AW]) == 0);

      always @(*)
        if (M_AXI_ARVALID[M])
          assert (((M_AXI_ARADDR[M*AW+:AW] ^ SLAVE_ADDR[M*AW+:AW]) & SLAVE_MASK[M*AW+:AW]) == 0);
      // }}}
    end
  endgenerate

  generate
    for (N = 0; N < NM; N = N + 1) begin : CORRELATE_OUTSTANDING
      // {{{
      always @(*)
        if (S_AXI_ARESETN && (swgrant[N] && (swindex[N] < NS))) begin
          assert((fm_awr_outstanding[N]
        - (S_AXI_AWREADY[N] ? 0:1)
        -((OPT_BUFFER_DECODER && dcd_awvalid[N]) ? 1:0)
        - (S_AXI_BVALID[N]  ? 1:0))
        ...
        end else if (S_AXI_ARESETN && (!swgrant[N] || (swindex[N] == NS))) begin
          if (!swgrant[N])
            assert(fm_awr_outstanding[N] ==
          (S_AXI_AWREADY[N] ? 0:1)
          +((OPT_BUFFER_DECODER && dcd_awvalid[N]) ? 1:0)
      ...
      always @(*)
        if (srgrant[N] && (srindex[N] < NS)) begin
          assert ((fm_rd_outstanding[N]  //17
          - (S_AXI_ARREADY[N] ? 0 : 1)  //1
          - ((OPT_BUFFER_DECODER && dcd_arvalid[N]) ? 1 : 0) - (S_AXI_RVALID[N] ? 1 : 0))  //0
  ...
  generate
    for (N = 0; N < NM; N = N + 1) begin : COVER_CONNECTIVITY_FROM_MASTER
      ...
      initial w_returns = 0;
      always @(posedge S_AXI_ACLK)
        if (!S_AXI_ARESETN) w_returns = 0;
        else begin
          w_returns <= {w_returns[2:0], 1'b0};
          if (S_AXI_BVALID[N] && S_AXI_BREADY[N] && !wgrant[N][NS]) w_returns[0] <= 1'b1;
        end

      initial whsreturn = 0;
      always @(posedge S_AXI_ACLK)
        if (!S_AXI_ARESETN) whsreturn <= 0;
        else whsreturn <= whsreturn || (&w_returns);

      // w_every is a connectivity test: Can we get a return from
      // every slave?
      initial w_every = 0;
      always @(posedge S_AXI_ACLK)
        if (!S_AXI_ARESETN) w_every <= 0;
        else if (!S_AXI_AWVALID[N]) w_every <= 0;
        else begin
          if (S_AXI_BVALID[N] && S_AXI_BREADY[N] && !wgrant[N][NS]) w_every[swindex[N]] <= 1'b1;
        end
      ...
      always @(posedge S_AXI_ACLK)
        if (!S_AXI_ARESETN) was_wevery <= 0;
        else was_wevery <= was_wevery || (&w_every);

      // err_wr_return is a test to make certain we can return a
      // bus error on the write channel.
      initial err_wr_return = 0;
      always @(posedge S_AXI_ACLK)
        if (!S_AXI_ARESETN) err_wr_return = 0;
        else if (wgrant[N][NS] && S_AXI_BVALID[N] && (S_AXI_BRESP[2*N+:2] == INTERCONNECT_ERROR))
          err_wr_return = 1;
...
`ifndef VERILATOR
      ...
      always @(*) cover (!swgrant[N] && err_wr_return);
`endif

      always @(*) if (S_AXI_BVALID[N]) assert (swgrant[N]);

      // r_returns is a speed check: Can we return one read
      // acknowledgment per clock cycle?
      initial r_returns = 0;
      always @(posedge S_AXI_ACLK)
        if (!S_AXI_ARESETN) r_returns = 0;
        else begin
          r_returns <= {r_returns[2:0], 1'b0};
          if (S_AXI_RVALID[N] && S_AXI_RREADY[N]) r_returns[0] <= 1'b1;
        end

      initial rhsreturn = 0;
      always @(posedge S_AXI_ACLK)
        if (!S_AXI_ARESETN) rhsreturn <= 0;
        else rhsreturn <= rhsreturn || (&r_returns);
      ...
      initial r_every = 0;
      always @(posedge S_AXI_ACLK)
        if (!S_AXI_ARESETN) r_every = 0;
        else if (!S_AXI_ARVALID[N]) r_every = 0;
        else begin
          if (S_AXI_RVALID[N] && S_AXI_RREADY[N]) r_every[srindex[N]] <= 1'b1;
        end
      ...
      always @(posedge S_AXI_ACLK)
        if (!S_AXI_ARESETN) was_revery <= 0;
        else was_revery <= was_revery || (&r_every);

      always @(posedge S_AXI_ACLK) if (S_AXI_RVALID[N]) assert ($stable(srindex[N]));

      initial err_rd_return = 0;
      always @(posedge S_AXI_ACLK)
        if (!S_AXI_ARESETN) err_rd_return = 0;
        else if (rgrant[N][NS] && S_AXI_RVALID[N] && (S_AXI_RRESP[2*N+:2] == INTERCONNECT_ERROR))
          err_rd_return = 1;
...
`ifndef VERILATOR
      ...
      always @(*) cover (!srgrant[N] && err_rd_return);  //@!
`endif
  ...
  always @(posedge S_AXI_ACLK)
    if (!S_AXI_ARESETN) multi_write_hit <= 0;
    else if (fm_awr_outstanding[0] > 2 && !wgrant[0][NS]) multi_write_hit <= 1;

  initial multi_read_hit = 0;
  always @(posedge S_AXI_ACLK)
    if (!S_AXI_ARESETN) multi_read_hit <= 0;
    else if (fm_rd_outstanding[0] > 2 && !rgrant[0][NS]) multi_read_hit <= 1;
  ...
  always @(*) cover (S_AXI_ARESETN && multi_read_hit & mrgrant == 0 && M_AXI_RVALID == 0);
  // }}}
  ////////////////////////////////////////////////////////////////////////
  //
  // Negation check
  ...
  always @(*) begin
    f_const_slave = NS;
    for (iM = 0; iM < NS; iM = iM + 1) begin
      if (((f_const_addr ^ SLAVE_ADDR[iM*AW+:AW]) & SLAVE_MASK[iM*AW+:AW]) == 0) f_const_slave = iM;
    end
  ...
  always @(*) f_araddr = S_AXI_ARADDR[f_const_source*AW+:AW];

  // The assumption check: assume our negated values are not found on
  // the inputs
  always @(*) begin
    if (S_AXI_AWVALID[f_const_source]) begin
    ...
    if (S_AXI_ARVALID[f_const_source]) begin
      assume (f_araddr != f_const_addr_n);
      assume (S_AXI_ARPROT[f_const_source*3+:3] != f_const_prot_n);
    end

    if (M_AXI_BVALID[f_const_slave] && wgrant[f_const_source][f_const_slave]) begin
      assume (m_axi_bresp[f_const_slave] != f_const_resp_n);
    end

    if (M_AXI_RVALID[f_const_slave] && rgrant[f_const_source][f_const_slave]) begin
  ...
  always @(*) begin
    ...
    if (r_rvalid[f_const_source] && rgrant[f_const_source][f_const_slave]) begin
      assert (r_rresp[f_const_source] != f_const_resp_n);
      assert (r_rdata[f_const_source] != f_const_data_n);
    end
    if (S_AXI_RVALID[f_const_source] && rgrant[f_const_source][f_const_slave]) begin
