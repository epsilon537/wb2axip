...
`endif
  ...
  always @(posedge i_clk)
    if (!i_reset && o_wb_stb && i_wb_stall) begin
      if (stall_timer <= OPT_TIMEOUT) stall_timer <= stall_timer + 1;
    end else stall_timer <= 0;
  ...
  always @(posedge i_clk)
    if (!i_reset && o_wb_cyc && !o_wb_stb && !i_wb_ack && !i_wb_err && !none_expected) begin
      if (wait_timer <= OPT_TIMEOUT) wait_timer <= wait_timer + 1;
    end else wait_timer <= 0;
  ...
  always @(posedge i_clk)
    if (i_reset || o_reset || o_wb_err || !i_wb_cyc) begin
      expected_returns <= 0;
      none_expected    <= 1;
    end else
      case ({
        skd_stb && !skd_stall, o_wb_ack
      })
        2'b10: begin
  ...
  // {{{
  generate
    if (OPT_SELF_RESET) begin : SELF_RESET

      initial o_reset = 1;
      always @(posedge i_clk)
        if (i_reset || o_fault) o_reset <= 1;
        else begin
          o_reset <= 0;
          if (o_wb_cyc && none_expected && (i_wb_ack || i_wb_err)) o_reset <= 1;
          if (timeout) o_reset <= 1;
        end
    end else begin : FORWARD_RESET

      always @(*) o_reset = i_reset || o_fault;
  ...
  always @(*) begin
    faulty_return = 0;
    if (expected_returns <= ((o_wb_stb && i_wb_stall) ? 1 : 0) + ((o_wb_ack || o_wb_err) ? 1 : 0))
      faulty_return = i_wb_ack || i_wb_err;
    if (i_wb_ack && i_wb_err) faulty_return = 1;
    if (!i_wb_cyc || !o_wb_cyc) faulty_return = 0;
  end
  // }}}
  ...
  initial o_fault = 0;
  always @(posedge i_clk)
    if (o_reset && !i_wb_cyc) o_fault <= 0;
    else begin
      if (o_wb_cyc && faulty_return) o_fault <= 1;
      if (i_wb_cyc && timeout) o_fault <= 1;
    end
  // }}}
  ...
  always @(posedge i_clk)
    if (i_reset || (o_wb_cyc && i_wb_err) || o_reset || o_fault || (i_wb_cyc && o_wb_err))
      o_wb_cyc <= 1'b0;
    else o_wb_cyc <= i_wb_cyc && (o_wb_cyc || i_wb_stb);
  // }}}
  ...
  always @(posedge i_clk)
    if (i_reset || (o_wb_cyc && i_wb_err) || o_reset || o_fault || !i_wb_cyc
      || (i_wb_cyc && o_wb_err))
      o_wb_stb <= 1'b0;
    else if (!o_wb_stb || !i_wb_stall) o_wb_stb <= skd_stb;
  // }}}
  ...
  // o_wb_idata
  // {{{
  always @(posedge i_clk) o_wb_idata <= i_wb_idata;
  // }}}
  ...
  always @(*) begin
    ...
    if (i_reset) skd_stall = 1'b1;
    if (o_fault) skd_stall = 1'b0;
    else if (o_reset) skd_stall = 1'b1;
  end
  // }}}
  ...
  always @(posedge i_clk)
    if (i_reset || !i_wb_cyc) begin
      o_wb_ack <= 1'b0;
      o_wb_err <= 1'b0;
    end else if (!o_reset && !o_fault) begin
      if (timeout || faulty_return || i_wb_err) begin
        o_wb_ack <= 1'b0;
...
`ifdef FORMAL
  ...
  always @(*) assert (none_expected == (expected_returns == 0));
  ...
  always @(*) if (i_wb_cyc && !o_wb_err && !o_fault) assert (expected_returns == fwbs_outstanding);

  generate
    if (F_OPT_FAULTLESS) begin : FAULTLESS_PROPERTIES
      // {{{
      ////////////////////////////////////////////////////////////////
      //
      ...
      always @(*) if (o_wb_err) assert (!o_wb_cyc);

      always @(*) sacks = fwbs_nacks + (o_wb_ack ? 1 : 0);

      always @(*) if (!o_wb_err && i_wb_cyc && o_wb_cyc) assert (sacks == fwbm_nacks);

      always @(posedge i_clk)
        if (!i_reset && i_wb_cyc && expected_returns > 0)
          assert (o_wb_cyc || o_wb_err);

      always @(*) mreqs = fwbm_nreqs + (o_wb_stb ? 1 : 0);

      always @(*) if (!o_wb_err && i_wb_cyc && o_wb_cyc) assert (fwbs_nreqs == mreqs);

      always @(*) if (i_wb_cyc && o_wb_cyc && fwbs_outstanding > 0) assert (i_wb_we == o_wb_we);

      always @(*) if (fwbs_nacks != 0 && i_wb_cyc) assert (o_wb_cyc || o_wb_err);
      // }}}
    end else begin
      // {{{
      ////////////////////////////////////////////////////////////////
      ...
      always @(posedge i_clk) cover (o_fault);

      always @(posedge i_clk) if (f_past_valid && $past(faulty_return)) cover (o_fault);

      always @(posedge i_clk) if (f_past_valid && $past(timeout)) cover (o_fault);
      ...
      if (OPT_SELF_RESET) begin
        ...
        initial faulted = 0;
        always @(posedge i_clk)
          if (i_reset) faulted <= 0;
          else if (o_fault) faulted <= 1;


        always @(posedge i_clk) cover (faulted && $fell(o_reset));

        always @(posedge i_clk) cover (faulted && !o_reset && o_wb_ack);
  ...
  always @(*) cover (!i_reset && fwbs_nacks > 4);
