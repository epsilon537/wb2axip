...
`else
  skidbuffer #(
      ...
      .i_data ({S_AXIS_TDATA, S_AXIS_TSTRB, S_AXIS_TKEEP, S_AXIS_TLAST}),
      .o_valid(pre_tvalid),
      .i_ready(pre_tready),
...
`endif
  ...
  always @(*) begin
    {pck_tdata, pck_tstrb, pck_tkeep} =
        pack(pre_tdata, pre_tstrb, (pre_tvalid) ? pre_tkeep : {(DW / 8) {1'b0}});
    pck_tlast = pre_tlast;
...
`else
  skidbuffer #(
      ...
      .i_valid(pre_tvalid),
      .o_ready(pre_tready),
      .i_data ({pck_count, pck_tdata, pck_tstrb, pck_tkeep, pck_tlast}),
      .o_valid(skd_valid),
      .i_ready(axis_ready),
      .o_data ({skd_count, skd_data, skd_strb, skd_keep, skd_last})
      // }}}
  );
`endif
  ...
  function [DW+DW/8+DW/8-1:0] pack;  // (i_data, i_strb, i_keep)
    ...
    begin
      ...
      for (rounds = 0; rounds < DW / 8; rounds = rounds + 1) begin
        for (ik = 0; ik < DW / 8 - 1; ik = ik + 1) begin
          if (!p_keep[ik]) begin
...
*/
  ...
  always @(*) begin
    axis_ready = 1;
    if ((mid_last || (skd_count + mid_fill >= TRIM_MAX)) && (M_AXIS_TVALID && !M_AXIS_TREADY))
      axis_ready = 0;
    if (!skd_valid) axis_ready = 0;
  end
  // }}}
  ...
  // {{{
  always @(posedge S_AXI_ACLK)
    if (!S_AXI_ARESETN) mid_last <= 0;
    else if (axis_ready) begin
      mid_last <= !next_last || (M_AXIS_TVALID && !M_AXIS_TREADY);
      if (mid_last) mid_last <= 1'b1;
      if (!skd_last) mid_last <= 1'b0;
    end else if (!M_AXIS_TVALID || M_AXIS_TREADY) mid_last <= 0;
  // }}}
  ...
  always @(posedge S_AXI_ACLK)
    if (!S_AXI_ARESETN) begin
      ...
      mid_strb <= 0;
      mid_keep <= 0;
    end else if (mid_last && (!M_AXIS_TVALID || M_AXIS_TREADY)) begin
      mid_data <= (axis_ready) ? skd_data : 0;
      mid_strb <= (axis_ready) ? skd_strb : 0;
      mid_keep <= (axis_ready) ? skd_keep : 0;
    end else if (skd_valid && skd_last && (!M_AXIS_TVALID || M_AXIS_TREADY)) begin
      if (skd_count + mid_fill <= TRIM_MAX) begin
        mid_data <= 0;
        mid_strb <= 0;
        mid_keep <= 0;
      end else begin
    ...
    end else if (axis_ready) begin
      if (mid_fill + skd_count >= TRIM_MAX) begin
        mid_data <= w_packed_data[2*DW-1:DW];
        mid_strb <= w_packed_strb[2*DW/8-1:DW/8];
        mid_keep <= w_packed_keep[2*DW/8-1:DW/8];
      end else begin
  ...
  always @(*) begin
    ...
    w_packed_data = {{(DW) {1'b0}}, mid_data} | ({{(DW) {1'b0}}, skd_data} << (mid_fill * 8));
    w_packed_strb = {{(DW / 8) {1'b0}}, mid_strb} | ({{(DW / 8) {1'b0}}, skd_strb} << (mid_fill));
    w_packed_keep = {{(DW / 8) {1'b0}}, mid_keep} | ({{(DW / 8) {1'b0}}, skd_keep} << (mid_fill));
  end
  // }}}
  // }}}
  ////////////////////////////////////////////////////////////////////////
  ...
  always @(*) begin
    ...
    if (mid_last || (skd_last && skd_valid)) next_valid = 1;
    if (skd_count + mid_fill == 0) next_valid = 0;
  end
  // }}}
  ...
  always @(*) begin
    next_last = mid_last;
    if (skd_valid && skd_last && (skd_count + mid_fill <= TRIM_MAX)) next_last = 1;
    if (!next_valid) next_last = 0;
  end
  // }}}
  ...
  always @(posedge S_AXI_ACLK)
    if (!S_AXI_ARESETN) M_AXIS_TVALID <= 0;
    else if (!M_AXIS_TVALID || M_AXIS_TREADY) M_AXIS_TVALID <= next_valid;
  // }}}
  ...
  always @(posedge S_AXI_ACLK)
    if (OPT_LOWPOWER && !S_AXI_ARESETN) begin
      ...
      if (OPT_LOWPOWER && !next_valid) begin
        M_AXIS_TDATA <= 0;
        M_AXIS_TSTRB <= 0;
        M_AXIS_TKEEP <= 0;
        M_AXIS_TLAST <= 0;
...
`ifdef FORMAL
  ...
  reg [F_COUNT-1:0] f_icount, f_ocount, f_ibeat, f_obeat, f_mcount, f_mbeat;
  ...
  always @(posedge S_AXI_ACLK)
    ...
    end else if ($past(S_AXIS_TVALID && !S_AXIS_TREADY)) begin
      assume (S_AXIS_TVALID);
  ...
  always @(*) if (S_AXI_ARESETN) assume (((~S_AXIS_TKEEP) & S_AXIS_TSTRB) == 0);

  // TLAST requires at least one beat
  always @(*) if (S_AXI_ARESETN && S_AXIS_TVALID && S_AXIS_TLAST) assume (S_AXIS_TKEEP != 0);
  ...
  // If TKEEP is low, TSTRB must be low as well
  always @(*) if (S_AXI_ARESETN && M_AXIS_TVALID) assert (((~M_AXIS_TKEEP) & M_AXIS_TSTRB) == 0);

  // Our output requirement is that if TLAST is ever low, TKEEP must
  // be all ones
  always @(*) if (S_AXI_ARESETN && M_AXIS_TVALID && !M_AXIS_TLAST) assert (&M_AXIS_TKEEP);

  always @(*)
    if (S_AXI_ARESETN && OPT_LOWPOWER && !M_AXIS_TVALID) begin
  ...
  // TLAST requires at least one beat
  always @(*) if (S_AXI_ARESETN && M_AXIS_TVALID && M_AXIS_TLAST) assert (M_AXIS_TKEEP != 0);
  ...
  always @(*) if (S_AXI_ARESETN) assert (mid_fill == {1'b0, $countones(mid_keep)});

  always @(*) if (S_AXI_ARESETN && mid_last) assert (mid_fill > 0);
  ...
  // be all ones
  generate
    for (gk = 1; gk < DW / 8; gk = gk + 1) begin
      always @(*) if (S_AXI_ARESETN && mid_fill > 0 && mid_keep[gk]) assert (&mid_keep[gk-1:0]);
    end
  endgenerate
  ...
  // {{{
  generate
    for (gk = 0; gk < DW / 8; gk = gk + 1) begin
      if (gk == 0) begin
        always @(*)
          if (S_AXIS_TVALID && S_AXIS_TKEEP[0] && f_icount == fc_count) begin
            assume (fc_data == S_AXIS_TDATA[7:0]);
            assume (fc_strb == S_AXIS_TSTRB[0]);
            if (fc_last) begin
              assume (S_AXIS_TKEEP[DW/8-1:1] == 0);
              assume (S_AXIS_TLAST);
            end else if (S_AXIS_TLAST) assume (S_AXIS_TKEEP[DW/8-1:1] != 0);
          end
      end else begin
        ...
        always @(*)
          if (S_AXIS_TKEEP[gk] && (f_icount
              ...
              + $countones(
                  S_AXIS_TKEEP[gk-1:0]
              ) == fc_count))  // Verilator lint_on  WIDTH
                  begin
            assume (S_AXIS_TDATA[gk*8+:8] == fc_data);
            assume (S_AXIS_TSTRB[gk] == fc_strb);
            if (fc_last) begin
              if (gk < DW / 8 - 1) assume (S_AXIS_TKEEP[DW/8-1:gk+1] == 0);
              assume (S_AXIS_TLAST);
            end else if (gk < DW / 8 - 1) begin
              if (S_AXIS_TLAST) assume (S_AXIS_TKEEP[DW/8-1:gk+1] != 0);
            end else assume (!S_AXIS_TLAST);
          end
      end
    end
  endgenerate
  ...
  generate
    for (gk = 0; gk < DW / 8; gk = gk + 1) begin
      if (gk == 0) begin
        always @(*)
          if (S_AXI_ARESETN && M_AXIS_TVALID && M_AXIS_TKEEP[0] && f_ocount == fc_count) begin
            assert (M_AXIS_TDATA[7:0] == fc_data);
            assert (M_AXIS_TSTRB[0] == fc_strb);
            if (fc_last) begin
              assert (M_AXIS_TKEEP[DW/8-1:1] == 0);
              assert (M_AXIS_TLAST);
            end else if (M_AXIS_TLAST) assert (M_AXIS_TKEEP[DW/8-1:1] != 0);
          end
      end else begin
        ...
        always @(*)
          if (S_AXI_ARESETN && M_AXIS_TKEEP[gk] &&
            ...
            if (fc_last) begin
              if (gk < DW / 8 - 1) assert (M_AXIS_TKEEP[DW/8-1:gk+1] == 0);
              assert (M_AXIS_TLAST);
            end else if (gk < DW / 8 - 1) begin
              if (M_AXIS_TLAST) assert (M_AXIS_TKEEP[DW/8-1:gk+1] != 0);
            end else assert (!M_AXIS_TLAST);
          end
      end
    end
  endgenerate
  ...
  // {{{
  always @(posedge S_AXI_ACLK)
    if (!S_AXI_ARESETN) f_icount <= 0;
    else if (S_AXIS_TVALID && S_AXIS_TREADY)
      // Verilator lint_off WIDTH
      f_icount <= f_icount + $countones(
          S_AXIS_TKEEP
      );
  // Verilator lint_on  WIDTH
  // }}}
  ...
  // {{{
  always @(posedge S_AXI_ACLK)
    if (!S_AXI_ARESETN) f_ocount <= 0;
    else if (M_AXIS_TVALID && M_AXIS_TREADY)
      // Verilator lint_off WIDTH
      f_ocount <= f_ocount + $countones(
          M_AXIS_TKEEP
      );
  // Verilator lint_on  WIDTH
  // }}}
  ...
  generate
    for (gk = 0; gk < DW / 8; gk = gk + 1) begin
      if (gk == 0) begin
        always @(*)
          if (S_AXI_ARESETN && mid_fill > 0 && f_mcount == fc_count) begin
            ...
            if (fc_last) begin
              assert (mid_fill == 1);
              assert (mid_last);
            end else if (mid_last) assert (mid_fill > 1);
          end else if (S_AXI_ARESETN && mid_fill == 0) begin
        ...
        always @(*)
          if (S_AXI_ARESETN && mid_fill > gk && (f_mcount + gk == fc_count)) begin
            ...
            if (fc_last) begin
              ...
              if (mid_last) assert (mid_fill > gk + 1);
            end else assert (!mid_last);
          end else if (S_AXI_ARESETN && mid_fill <= gk) begin
  ...
  // {{{
  always @(posedge S_AXI_ACLK)
    if (!S_AXI_ARESETN) f_ibeat <= 0;
    else if (S_AXIS_TVALID && S_AXIS_TREADY) begin
      // Verilator lint_off WIDTH
      f_ibeat <= f_ibeat + $countones(S_AXIS_TKEEP);
      // Verilator lint_on  WIDTH
      if (M_AXIS_TLAST) f_ibeat <= 0;
    end
  // }}}
  ...
  // {{{
  always @(posedge S_AXI_ACLK)
    if (!S_AXI_ARESETN) f_obeat <= 0;
    else if (M_AXIS_TVALID && M_AXIS_TREADY) begin
      // Verilator lint_off WIDTH
      f_obeat <= f_obeat + $countones(M_AXIS_TKEEP);
      // Verilator lint_on  WIDTH
      if (M_AXIS_TLAST) f_obeat <= 0;
    end
  // }}}
  // }}}
  ////////////////////////////////////////////////////////////////////////
  ...
  always @(posedge S_AXI_ACLK)
    if (S_AXI_ARESETN && $past(S_AXI_ARESETN))
      cover (M_AXIS_TVALID && M_AXIS_TREADY && M_AXIS_TLAST && $past(
          M_AXIS_TVALID && M_AXIS_TREADY && M_AXIS_TLAST
      ));

  always @(posedge S_AXI_ACLK)
    if (S_AXI_ARESETN && $past(S_AXI_ARESETN))
      cover (M_AXIS_TVALID && !M_AXIS_TLAST && $past(
          M_AXIS_TVALID && M_AXIS_TREADY && M_AXIS_TLAST
      ));
